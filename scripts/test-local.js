...
async function testLocalFiles() {
  console.log(`${colors.cyan}Testing Local HTML Files${colors.reset}\n`);
  ...
  for (const testFile of testFiles) {
    try {
      ...
      console.log(`\n${colors.blue}Testing:${colors.reset} ${testFile.name}`);
      console.log(`${colors.blue}File:${colors.reset} ${testFile.path}`);
      ...
      if (jsErrors.length > 0) {
        console.log(`${colors.yellow}⚠${colors.reset} JS Errors: ${jsErrors.length}`);
      }

      console.log(`${colors.green}✓${colors.reset} ${testFile.name}`);
